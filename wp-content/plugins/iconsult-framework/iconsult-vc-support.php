...
***    ADD VC SC 1 :: COUNTER     ***
**************************************/
if(!function_exists("bind_theme_counter")){
	function bind_theme_counter( $atts, $content = null ) {
...
add_shortcode('bind_theme_counter', 'bind_theme_counter');
...
***    ADD VC SC 2 :: TEAM     ***
**************************************/
if(!function_exists("bind_theme_team")){
	function bind_theme_team( $atts, $content = null ) {
...
add_shortcode('bind_theme_team', 'bind_theme_team');
...
******************************************/
if(!function_exists("bind_sc_link_url")){
	function bind_sc_link_url( $atts, $content = null ) {
		$link_html = '';
		$a_open  = '';
		$a_close = '';
		extract( shortcode_atts( array( 
		...
		echo '<style>.vc_link_color_'.$time_start[1].' { '.$linkurl__link_color.' }</style>';
...
add_shortcode('bind_sc_link_url', 'bind_sc_link_url');


/*************************************
***  ADD VC SC 4 :: TEXT WITH ICON ***
**************************************/
if(!function_exists("bind_sc_icon_with_text")){
	function bind_sc_icon_with_text( $atts, $content = null ) {
			...
			$link = (function_exists("vc_build_link") ? vc_build_link($link) : $link);
			 ...
			 if( $link_icon == 'no' || $link_icon == 'both' ) {  
				if( isset($link['title']) && $link['title'] != '' ) {
					...
					$link_html = '<p style="padding-top:10px;font-size:13px;text-transform: capitalize;letter-spacing: 0.6px;"> <a href="'.$link['url'].'" class="more-link hvr-icon-wobble-horizontal" style=" '.$vctexticon__vc_link_font_weight.' '.$vctexticon__link_color.' '.$vctexticon__vc_link_font_transform.' '.$vctexticon__vc_link_font_size.' " target="'.$link_target.'">'.$link['title'].'&nbsp;&nbsp;<i class="fa fa-arrow-right hvr-icon"></i></a> </p>';
				} else {  
					$link_html = '';
				}
			} 
		...
		if( $display_icon_position == 'left_from_title' ) {
			...
			$return .= '<div class="icon_with_text_title '.$box_css_animation.'" style="padding:'.$icon_with_text_box_padding.'; '.$icon_text__box_background_color.' '.$icon_text__box_border_radius.' '.$box_border_color.'">
...
add_shortcode('bind_sc_icon_with_text', 'bind_sc_icon_with_text');
...
***    ADD VC SC 5 :: BUTTON   ***
******************************************/
if(!function_exists("bind_sc_button_url")){
	function bind_sc_button_url( $atts, $content = null ) {
		...
		echo '<style>.vc_btm_'.$time_start[1].' { '.$vcbtm__button_color.' '.$vcbtm__link_color.' }</style>';
...
add_shortcode('bind_sc_button_url', 'bind_sc_button_url');
...
******************************************/
if(!function_exists("bind_sc_message_box")){
	function bind_sc_message_box( $atts, $content = null ) {
...
add_shortcode('bind_sc_message_box', 'bind_sc_message_box');
...
******************************************/
if(!function_exists("theme_portfolio_list")){
	function theme_portfolio_list( $atts, $content = null ) {
		...
		if( isset($portfolio_type) && $portfolio_type != '') {
			if( $portfolio_type == 'FitRows' ) {
				$image_handler_size = 'iconsult-image-700x525';
		...
		$return = '<style>.title_color_'.$time_start[1].' { '.$newstyle__link_color.' }</style><span></span>';
		...
		if( isset($portfolio_shorting) && $portfolio_shorting != '' && $portfolio_shorting == 'yes') {
			...
			if( ! empty($tax_terms) ) { 
				...
				$return .= '<li style="'.$filter_padding_align_li.'" data-filter-masnory="*" class="selected"><span>'. esc_html__( 'All', 'iconsult-framework' ) .'</span></li>';
		...
		if($wp_query->have_posts()) {
			...
			while($wp_query->have_posts()) : $wp_query->the_post();
				...
				$return .= '">';
					...
					$vcportfolio__link_cat_color = '';
					if( isset( $link_cat_color ) && $link_cat_color != "" ) {
						$vcportfolio__link_cat_color = 'color:'.$link_cat_color.';';
					}
					if( $show_categories == 'yes' ) $return .= $no_title_br.'<span style=" '.$vcportfolio__link_cat_color.' ">'. implode(', ', $out ).' </span>';
					$return .= '</div>';
				$return .= '</div>';
			 endwhile;
			...
			$return .= '<p class="no-records"> '. esc_html__('Sorry, no posts matched your criteria.', 'iconsult-framework') .'</p>';
		}
		$return .= '</div>';
		...
		if( isset($show_load_more_margin) && $show_load_more_margin != '' ) $show_load_more_margin = $show_load_more_margin;
			...
			$return .= '<div class="portfolio_paging"><span rel="' . $wp_query->max_num_pages . '" class="ajax_load_more_post custom-botton hvr-icon-spin" style="padding: 9px 37px 10px 24px;">' . get_next_posts_link(esc_html__('Show more', 'iconsult-framework'), $wp_query->max_num_pages) . '&nbsp;&nbsp;<i class="fas fa-sync-alt hvr-icon"></i></span></div>';
			$return .= '<div class="portfolio_paging_loading"><a href="javascript: void(0)" class="custom-botton" style="padding: 9px 37px 10px 24px;">'.esc_html__('Loading...', 'iconsult-framework').'</a></div>';
			$return .= '</div>';
		}
...
add_shortcode('theme_portfolio_list', 'theme_portfolio_list');
...
**************************************/
if(!function_exists("theme_user_testimonial")){
	function theme_user_testimonial( $atts, $content = null ) {
		...
		$return = '<div class="testimonial-slider"><ul class="slides">';
			...
			if( $category == "" ) { 
				$args = array(
					'post_type' => 'iconsult_testimonial',
				...
				if($wp_query->have_posts()) { 
					...
					$return .= '<li>
									<div class="testo_img_wrap">
										<img alt="" src="'.get_post_meta( $wp_query->post->ID, '__iconsult_testimonial_user_image', true ).'">
									</div>
									...
									'.get_post_meta( $wp_query->post->ID, '__iconsult_testimonial_user_message', true ).'
									 </p>
									 <h5 style=" '.$testo__client_name_color.' ">'.get_post_meta( $wp_query->post->ID, '__iconsult_testimonial_user_name', true ).' 
									 <br><span class="author_designation" style=" '.$testo__client_designation_color.' "> '.get_post_meta( $wp_query->post->ID, '__iconsult_testimonial_user_designation', true ).'</span>
									 </h5>
								</li>';
						 
					}
				} else {
					$return .= '<li><p class="no-records"> '. esc_html__('Sorry, no posts matched your criteria.', 'iconsult-framework') .'</p></li>';
				}
				wp_reset_postdata(); 
...
add_shortcode('theme_user_testimonial', 'theme_user_testimonial');
...
**************************************/
if (!function_exists('theme_shop_product_list')) {
    function theme_shop_product_list($atts, $content = null) {
        $args = array(
            "post_per_page"     => "-1",
            "columns"           => "",
            "category"          => "",
        ...
        while ($q->have_posts()) : $q->the_post();
            ...
            $image = '';
            if ( has_post_thumbnail() ) {
                $image = get_the_post_thumbnail( $q->post->ID, 'iconsult-image-woo-600x600', array( 'class' => 'hvr-float' ) );
            }
    ...
    add_shortcode('theme_shop_product_list', 'theme_shop_product_list');
}
...
**************************************/
if (!function_exists('theme_shop_product_list_masonry')) {
    function theme_shop_product_list_masonry($atts, $content = null) {
        $args = array(
            "per_page"                  => "-1",
            "columns"                   => "",
            "category"                  => "",
    ...
    add_shortcode('theme_shop_product_list_masonry', 'theme_shop_product_list_masonry');
}

/*************************************
***    ADD VC SC 11 :: LATEST POST ***
**************************************/
if (!function_exists('theme_blog_post')) {
    function theme_blog_post($atts, $content = null) {
		...
		echo '<style>.vc_blog_title_color'.$time_start[1].' { '.$latestpost__blog_title_color_htag.' }</style>';
		...
		if( $format == 'gallery' ) {
			...
			if( $count_array >= 1 && $array_id[0] != ''  ) { 
				...
				foreach($array_id as $img_id){ 
					$return .= '<li><a href="'.get_permalink().'">'. wp_get_attachment_image( $img_id, 'iconsult-image-700x525' ).'</a></li>';
				}
				$return .= '</ul></div>';
				
			} else {
				$return .= '<a href="'.get_permalink().'">'.get_the_post_thumbnail(get_the_ID(), 'iconsult-image-700x525').'</a>'; 
			}
			
		} else if( $format == 'quote' ) {
				
			$blockquote = get_post_meta( get_the_ID(), '__iconsult_post_format_quote', true );
			if( !empty($blockquote) ) $return .= '<blockquote>'. $blockquote .'</blockquote>';
			
		// HANDLE AUDIO	
		} else if( $format == 'audio' ) {
				
			$post_audio = get_post_meta( get_the_ID(), '__iconsult_post_format_audio', true );
			if( !empty($post_audio) ) {
				
				 $return .= '<audio class="blog_audio" src="'.get_post_meta( get_the_ID(), '__iconsult_post_format_audio', true ).'" controls="controls">';
		...
		$bind_post_format_quote_fix = get_post_meta( get_the_ID(), '__iconsult_post_format_quote', true );
		if( $format == 'gallery' ) {
			$gallery_content = do_shortcode($filtered_content);
			$return .= get_iconsult__chk_excerpt_content( $q->post_excerpt, $gallery_content, $display_continue_read, $blog_content_limit, $format, $excerpt_content_limit );
			
		} else if( $format == 'quote' && !empty($bind_post_format_quote_fix) ) {
			
		} else if( $format == 'audio' ) {
			$return .= get_iconsult__chk_excerpt_content( $q->post_excerpt, $filtered_audio_content, $display_continue_read, $blog_content_limit, $format, $excerpt_content_limit );
			
		} else if( $format == 'video' ) {
			$return .= get_iconsult__chk_excerpt_content( $q->post_excerpt, $filtered_video_content, $display_continue_read, $blog_content_limit, $format, $excerpt_content_limit );
			
		} else {  
			$return .= get_iconsult__chk_excerpt_content( $q->post_excerpt, $blog_content, $display_continue_read, $blog_content_limit, $format, $excerpt_content_limit );
		}
        $return .= '</div>';
		}
		...
		if ( $show_load_more == "yes" && ($q->max_num_pages != 0) && ($q->found_posts > $number_of_posts) ) {
			...
			$return .= '<div class="blog_vc_paging"><span rel="' . $q->max_num_pages . '" class="ajax_load_more_post_record custom-botton hvr-icon-spin" style="padding: 9px 37px 10px 24px;">' . get_next_posts_link(esc_html__('Show more', 'iconsult-framework'), $q->max_num_pages) . '&nbsp;&nbsp;<i class="fas fa-sync-alt hvr-icon"></i></span></div>';
			$return .= '<div class="blog_vc_paging_loading"><a href="javascript: void(0)" class="custom-botton" style="padding: 9px 37px 10px 24px;">'.esc_html__('Loading...', 'iconsult-framework').'</a></div>';
			$return .= '</div>';
		}
    ...
    add_shortcode('theme_blog_post', 'theme_blog_post');
}
...
**************************************/
if (!function_exists('theme_image_slider')) {
    function theme_image_slider($atts, $content = null) {
    ...
    add_shortcode('theme_image_slider', 'theme_image_slider');
}
...
**************************************/
if (!function_exists('theme_bbpress_login')) {
	function theme_bbpress_login($atts, $content = null) {
	...
	<fieldset class="bbp-form">
		...
		<div class="bbp-username">
			<label for="user_login" style="color:'.$text_color.'">'. esc_html__( 'Username', 'iconsult-framework' ).': </label>
			<input type="text" name="log" value="'. bbp_sanitize_val( 'user_login', 'text' ).'" size="20" id="user_login" />
		</div>

		<div class="bbp-password">
			<label for="user_pass" style="color:'.$text_color.'">'. esc_html__( 'Password', 'iconsult-framework' ).': </label>
		...
		<div class="bbp-remember-me">
			...
			<label for="rememberme"  style="color:'.$text_color.'">'. esc_html__( 'Keep me signed in', 'iconsult-framework' ) .'</label>
		</div>
		...
		if( !empty($register_link['title']) ) {
			$return .= '<div><a href="'.$register_link['url'].'" target="'.$register_link['target'].'" class="more-link hvr-icon-wobble-horizontal">'.$register_link['title'].'&nbsp;&nbsp;<i class="fa fa-arrow-right hvr-icon"></i></a></div>';
		}
		
		if( !empty($lost_password_link['title']) ) {
			$return .= '<div><a href="'.$lost_password_link['url'].'" target="'.$lost_password_link['target'].'" class="more-link hvr-icon-wobble-horizontal">'.$lost_password_link['title'].'&nbsp;&nbsp;<i class="fa fa-arrow-right hvr-icon"></i></a></div>';
		}
	...
	add_shortcode('theme_bbpress_login', 'theme_bbpress_login');
}
...
**************************************/
if (!function_exists('theme_bbpress_register')) {
	function theme_bbpress_register($atts, $content = null) {
	...
	<fieldset class="bbp-form">
		<legend>'.esc_html__( 'Create an Account', 'iconsult-framework' ).'</legend>
		...
		<div class="bbp-username">
			<label for="user_login">'.esc_html__( 'Username', 'iconsult-framework' ).':</label>
			<input type="text" name="user_login" value="'. bbp_sanitize_val( 'user_login' ).'" size="20" id="user_login" />
		</div>

		<div class="bbp-email">
			<label for="user_email">'.esc_html__( 'Email', 'iconsult-framework' ).': </label>
		...
		<div class="bbp-submit-wrapper">
			<button type="submit" name="user-submit" class="custom-botton" style="background:'.$button_bg_color.';color:'.$button_text_color.'">'.esc_html__( 'Register', 'iconsult-framework' ).'</button>
...
';
	...
	add_shortcode('theme_bbpress_register', 'theme_bbpress_register');
}
...
**************************************/
if (!function_exists('theme_bbpress_lost_password')) {
	function theme_bbpress_lost_password($atts, $content = null) {
		...
		<fieldset class="bbp-form">
			<legend>'.esc_html__( 'Lost Password', 'iconsult-framework' ).'</legend>
			<div class="bbp-username">
				<p>
					<label for="user_login">'.esc_html__( 'Username or Email', 'iconsult-framework' ).': </label>
					<input type="text" name="user_login" value="" size="20" id="user_login" />
				</p>
			</div>
			'. do_action( 'login_form', 'resetpass' ) .'
			<div class="bbp-submit-wrapper">
				<button type="submit" name="user-submit" class="custom-botton" style="background:'.$button_bg_color.';color:'.$button_text_color.'">'.esc_html__( 'Reset My Password', 'iconsult-framework' ).'</button>
	...
	add_shortcode('theme_bbpress_lost_password', 'theme_bbpress_lost_password');
}
...
**************************************/
if (!function_exists('theme_knowledgebase_articles')) {
	function theme_knowledgebase_articles($atts, $content = null) {
	...
	global $iconsult_theme_options;	
		...
		$tax_terms = get_terms('iconsultkbcat', $args);
	    ...
	    $return .= '<div class="col-md-'.$col_md.' col-sm-12 masonry-item body-content"> 
				  ...
				  <a href="'.esc_attr(get_term_link($tax_term, 'iconsultkbcat')).'">'; 
         ...
         $return .= '<div style="padding:10px 0px;"> <a href="'. esc_attr(get_term_link($tax_term, 'iconsultkbcat')).'" class="custom-link hvr-icon-wobble-horizontal">
	...
	add_shortcode('theme_knowledgebase_articles', 'theme_knowledgebase_articles');
}
...
**************************************/
if(!function_exists("theme_knowledgebase_widget_cat")){
	function theme_knowledgebase_widget_cat( $atts, $content = null ) {
		...
		$categories = get_categories('taxonomy=iconsultkbcat&post_type=iconsult_kb');
		...
		foreach ($categories as $category) {
			...
			if( $category->category_parent  == 0  ) {
				...
				$subcategories = get_categories('taxonomy=iconsultkbcat&post_type=iconsult_kb&child_of='.$category->term_id.'');
...
add_shortcode('theme_knowledgebase_widget_cat', 'theme_knowledgebase_widget_cat');
...
**************************************/
if(!function_exists("theme_knowledgebase_widget_articles")){
	function theme_knowledgebase_widget_articles( $atts, $content = null ) {
...
add_shortcode('theme_knowledgebase_widget_articles', 'theme_knowledgebase_widget_articles');
...
**************************************/
if(!function_exists("theme_faq_articles")){
	function theme_faq_articles( $atts, $content = null ) {
...
add_shortcode('theme_faq_articles', 'theme_faq_articles');
...
**************************************/
if(!function_exists("theme_image_frame")){
	function theme_image_frame( $atts, $content = null ) {
		...
		if (is_numeric($image_frame) && isset($image_frame)) {
			...
			//$image_src = wp_get_attachment_image($image_frame, 'iconsult-image-700x525', false);
...
add_shortcode('theme_image_frame', 'theme_image_frame');
...
**************************************/
if(!function_exists("portfolio_images")){
	function portfolio_images( $atts, $content = null ) {
...
add_shortcode('portfolio_images', 'portfolio_images');
...
if(!function_exists("social_share")){
	function social_share( $atts, $content = null ) {
		...
		$return = '';
		if($display_social_share == true) $return = iconsult__portfolio_social_share(get_permalink(), true, $position); 
...
******************************************/
if(!function_exists("theme_services_list")){
	function theme_services_list( $atts, $content = null ) {
		$filter_padding_align_li = $padding_zero_class = '';
		global $post;
		extract( shortcode_atts( array( 
		...
		if( isset($services_shorting) && $services_shorting != '' && $services_shorting == 'yes') {
			...
			$tax_terms = get_terms('iconsultsvscat', $args);
			$tax_terms = wp_list_filter($tax_terms,array('parent'=>0));
...
add_shortcode('theme_services_list', 'theme_services_list');	
}
...
***    ADD VC SC 22 :: PAGES     ***
**************************************/
if(!function_exists("theme_pages_list")){
	function theme_pages_list( $atts, $content = null ) {
		global $post;
		$currentpost = $post->ID;
		$class = '';
		extract( shortcode_atts( array( 
		...
		if( $post_type == 'iconsult_services' ) $category_name = 'iconsultsvscat';
		else if( $post_type == 'iconsult_portfolio' ) $category_name = 'iconsultptfocategory';
		else $category_name = '';
...
add_shortcode('theme_pages_list', 'theme_pages_list');	
}
...
***    ADD VC SC 23 :: VIDEO     ***
**************************************/
if(!function_exists("iconsult__theme_popup")){
	function iconsult__theme_popup( $atts, $content = null ) {
		global $post;
		$currentpost = $post->ID;
		$class = '';
		extract( shortcode_atts( array( 
...
add_shortcode('iconsult__theme_popup', 'iconsult__theme_popup');	
}
