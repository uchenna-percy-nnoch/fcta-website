...
// excerpt
if( !empty( $iconsult_theme_options['blog_default_charater_limit_excerpt'] ) ) {
	$excerpt_content_limit = $iconsult_theme_options['blog_default_charater_limit_excerpt'];
} else {
	$excerpt_content_limit = 25;
}
// blog main content		
if( !empty( $iconsult_theme_options['blog_default_charater_limit_blog_main_content'] ) ) {
	$blog_content_limit = $iconsult_theme_options['blog_default_charater_limit_blog_main_content'];
} else {
...
$display_status = '';
if( $format == 'quote' ) { 
	if( $iconsult_theme_options['blog_post_title_quote_status'] == false ){  
		$display_status = 1; 
	} else { 
		$display_status = 2; 	
	} 
	// continue read status
	if( $iconsult_theme_options['blog_post_title_quote_morelink_status'] == false ){  
...
<div id="post-<?php the_ID(); ?>" <?php post_class('blog'); ?>>
	...
	?>
        <div class="entry-header">
            <?php if ( is_sticky() && is_home() && ! is_paged() ) : ?>
                <span class="sticky-post"><?php esc_html_e( 'Featured', 'iconsult' ); ?></span>
            <?php endif; ?>
	...
	<?php
		...
		if ( is_single() )  {  
				if( $iconsult_theme_options['blog_post_excerpts_status'] == true ) iconsult__excerpt();
				if( $format == 'gallery' ) {
			...
			$bind_post_format_quote = get_post_meta( get_the_ID(), '__iconsult_post_format_quote', true );
			if( $format == 'gallery' ) {
				$gallery_content = do_shortcode($filtered_content);
				iconsult__chk_excerpt_content( $post->post_excerpt, $gallery_content, $display_morelink_status, $blog_content_limit, $format, $excerpt_content_limit );
				
			} else if( $format == 'audio' ) {
				iconsult__chk_excerpt_content( $post->post_excerpt, $filtered_audio_content, $display_morelink_status, $blog_content_limit, $format, $excerpt_content_limit );
				
			} else if( $format == 'video' ) {
				iconsult__chk_excerpt_content( $post->post_excerpt, $filtered_video_content, $display_morelink_status, $blog_content_limit, $format, $excerpt_content_limit );
				
			} else if( $format == 'quote' && !empty($bind_post_format_quote) ) {
				
			} else {  
				iconsult__chk_excerpt_content( $post->post_excerpt, $post_content, $display_morelink_status, $blog_content_limit, $format, $excerpt_content_limit );
			}
